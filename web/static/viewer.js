...
// Adapted from the ensemble site viewer, which reads a local JSONL file.
// The public interface is the same; only the data source changes.
...
(function () {
  ...
  // Convert a server event (with sequence_number, kind, payload) to the
  // viewer's internal format (with tick, actor, payload.kind).
  function convertEvent(ev) {
    const payload = ev.payload || {};
    return {
      tick: ev.sequence_number,
      actor: payload.actor || '',
      payload: Object.assign({ kind: ev.kind }, payload),
    };
  }
  ...
  function renderDiff(slice) {
    ...
    for (const e of slice) {
      ...
      if (k === 'state_diff' && e.payload.diff) {
        ...
        rows.push({ who: e.actor || '?', table: d.table || d.field || '', field: d.field || '', old: str(d.old), new: str(d.new) });
      }
    }
  ...
  function renderChat(slice) {
    ...
    for (const e of slice) {
      ...
      if (k === 'user_message') {
        ...
        html.push(`<div class="chat-tool ${kind === 'user' ? 'right' : 'left'}"><span class="chat-tool-tag">[tool]</span> <strong>${esc(who)}</strong> called <code>${esc(p.name)}</code><div class="chat-tool-args">${renderArgs(p.args)}</div></div>`);
      } else if (k === 'tool_result') {
        html.push(`<div class="chat-tool ${kind === 'user' ? 'right' : 'left'}"><span class="chat-tool-tag">[result]</span> <code>${esc(p.name)}</code> <div class="chat-tool-res">${renderResult(p.result)}</div></div>`);
      } else if (k === 'cost') {
        ...
        const d = p.diff;
        html.push(`<div class="chat-note">[state] ${esc(`${d.table || d.field || 'state'}.${d.field || ''}: ${str(d.old)} -> ${str(d.new)}`)}</div>`);
      } else if (k === 'system') {
        html.push(`<div class="chat-note">${esc(p.note || '')}</div>`);
      }
    }
